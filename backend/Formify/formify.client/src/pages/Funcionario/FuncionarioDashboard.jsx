    ...
    useEffect(() => {
        const fetchForms = async () => {
            try {
                ...
                console.error('Erro na integracao:', error);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    return (
        <div className="min-h-[calc(100vh-140px)] space-y-8 flex flex-col">
            <div className="flex flex-col gap-4">
                <h2 className="text-3xl font-bold text-text-h">Area do Funcionario</h2>
            ...
            <div className="rounded-lg flex-1 flex flex-col">
                {isLoading ? (
                    <div className="text-center py-12 text-text">A carregar formularios...</div>
                ) : filteredAndSortedForms.length === 0 ? (
                    <div className="rounded-lg border-2 border-dashed border-accent-border bg-accent-bg px-8 py-12 text-center">
                        <p className="text-xl font-semibold text-text-h">Nenhum formulario disponivel</p>
                        <p className="mt-2 text-text">De momento nao existem formularios para funcionarios.</p>
                    </div>
                ) : (
                    <>
                        ...
                        <div className="mt-auto pt-6 flex items-center justify-between">
                            ...
                            <span className="text-sm text-text">Pagina {currentPage} de {totalPages}</span>
                            <button
                                type="button"
